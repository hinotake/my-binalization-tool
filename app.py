...
def get_gray_assets(img_bytes):
    # bytesからPILで読み込み、RGBに統一、Numpy配列化、グレースケール変換
...
if input_mode == "サンプル画像を使う":
    if SAMPLE_PATH.exists():
        img_bytes = SAMPLE_PATH.read_bytes()
        img_name = "sample.jpg"
        img_size = len(img_bytes)
    else:
        st.error("sample.jpg が見つかりません。app.py と同じ階層に配置してください。")
...
if img_bytes is not None:
    ...
    img_array, gray_img, display_gray_pil = get_gray_assets(img_bytes)
    ...
    m4.metric("Size", f"{img_size / 1024:.1f}KB")

    st.caption(f"現在の画像：{img_name}")
    ...
    bar_chart = alt.Chart(hist_df).mark_bar(
        color="black",
        binSpacing=0
    ).encode(
        x=alt.X(
            "輝度:Q",
            scale=alt.Scale(domain=[0, 255], padding=0),
            axis=alt.Axis(title="輝度 (0-255)")
        ),
        y=alt.Y(
            "頻度:Q",
            axis=alt.Axis(title="画素数")
        ),
    ...
    rule = alt.Chart(hist_df).mark_rule(
        color="red",
        size=2
    ).encode(
        x="輝度:Q"
    ...
    band_rect = alt.Chart(band_data).mark_rect(
        stroke="black",
        strokeWidth=1
    ).encode(
        x=alt.X(
    ...
    band_text = alt.Chart(band_data).mark_text(
        baseline="middle",
        fontWeight="bold"
    ).encode(
        x=alt.X("mid:Q"),
    ...
    with col1:
        ...
        if coords:
            ...
            if 0 <= x < width and 0 <= y < height:
                ...
                cap_html += (
                    f'<div class="pixel-chip" '
                    f'style="background-color: rgb({v},{v},{v}); '
                    f'color: {txt_color}; border: 1px solid #ccc;">'
                )
                cap_html += (
                    f'輝度: {v} '
                    f'<span style="font-size:0.8rem; opacity:0.8; margin-left:8px;">'
                    f'({x}, {y})'
                    f'</span></div>'
                )
        else:
    ...
    with col2:
        ...
        st.markdown(
            f'<div class="bottom-cap-row">'
            f'<div class="img-cap">2値化結果 (しきい値: {st.session_state.threshold})</div>'
            f'</div>',
            unsafe_allow_html=True
        )

else:
    st.info("サンプル画像を選ぶか、画像をアップロードしてください。")
