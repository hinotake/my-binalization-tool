...
with st.sidebar:
    ...
    st.markdown("(c) Takeyoshi Noriyuki")
    st.markdown("[Official Site](https://www.hinode.ed.jp/share/takeyoshi/)")
    st.divider()
    st.info("このツールは、画像の輝度ヒストグラムを元に、直感的に2値化（白黒化）のしきい値を決定するためのものです。")
...
if uploaded_file:
    ...
    hist_df = pd.DataFrame({"輝度": bin_edges[:-1].astype(int), "頻度": hist_values})

    # 2. Altairグラフの設定
    click_selection = alt.selection_point(name="pts", encodings=['x'], on='click', nearest=True, value=127)
    ...
    chart = alt.layer(bar, rule).add_params(click_selection).properties(
        width="container", height=180, title=""
    )
    ...
    if select_event and "selection" in select_event:
        ...
        if points and len(points) > 0:
            threshold = int(list(points[0].values())[0])
...
st.markdown(
    """
    <div class="footer">
        <p>(c) Takeyoshi Noriyuki | <a href="https://www.hinode.ed.jp/share/takeyoshi/" target="_blank">Web Site</a></p>
    </div>
    """,
    unsafe_allow_html=True
)
